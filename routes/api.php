...
Route::get('callback/notusedanymoreadgatemedia', function(Request $request) {
...
Route::middleware('auth')->prefix('wallet')->group(function() {
    Route::post('getDepositWallet', function(Request $request) {
        ...
        $apikey = 'V68WSXK-8GQMEJG-GQFEYHR-HT02EYS';
        $ipn = 'https://bitsarcade.com/api/callback/nowpayments';
        //$ipn = $currency->option('ipn');
        $price_amount = round($mindepositusd + 3.1, 2); //(usd, eur)
       ...
       if($responseResult->pay_currency == 'xrp' || $responseResult->pay_currency == 'bnb') {
        ...
        return success([
            ...
            'mindepositusd' => round($mindepositusd + 3.1, 2),
            'wallet' => $wallet
        ]); 
    }
    });
