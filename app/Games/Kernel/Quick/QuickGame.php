...
abstract class QuickGame extends Game {
    ...
    public function process(Data $data) {
        ...
        if(!$data->demo()) {
            ...
            if($currency == 'xBLZD'){
            $usd_wager = floatval(number_format($data->bet() * WalletController::rateDollarxBLZD(), 2, '.', ''));
            $stats->update([
                'bets_xBLZD' => $stats->bets_xBLZD + 1,
                'wins_xBLZD' => $stats->wins_xBLZD + ($result->profit() > 0 ? ($result->multiplier() < 1 ? 0 : 1) : 0),
                'loss_xBLZD' => $stats->loss_xBLZD + ($result->profit() > 0 ? ($result->multiplier() < 1 ? 1 : 0) : 1),
                'wagered_xBLZD' => $stats->wagered_xBLZD + $data->bet(),
                'profit_xBLZD' => $stats->profit_xBLZD + ($result->profit() > 0 ? ($result->multiplier() < 1 ? -($data->bet()) : ($result->profit())) : -($data->bet()))
            ]);
            }
            if($currency == 'bnb'){
