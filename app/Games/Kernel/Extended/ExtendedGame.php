...
abstract class ExtendedGame extends Game {
    ...
    public function process(Data $data) {
        ...
        if(!$data->guest()){
            ...
            if($data->currency() == 'xBLZD'){
            $stats->update([
                'bets_xBLZD' => $stats->bets_ltc + 1,
                'wagered_xBLZD' => $stats->wagered_ltc + $data->bet()
            ]);
            }
...
*/
    ...
    public function finish(\App\Game $game) {
        ...
        if($game->user != null) {
            ...
            if($game->currency == 'xBLZD'){
            $stats->update([
                'wins_xBLZD' => $stats->wins_xBLZD + ($game->status === 'lose' ? 0 : 1),
                'loss_xBLZD' => $stats->loss_xBLZD + ($game->status === 'lose' ? 1 : 0),
                'profit_xBLZD' => $stats->profit_xBLZD + ($game->status === 'lose' ? -($game->wager) : ($game->wager * $game->multiplier))
            ]);
            }
