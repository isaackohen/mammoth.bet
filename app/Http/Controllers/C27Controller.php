...
class C27Controller extends Controller
{
    ...
    public function withdrawAndDeposit(Request $request)
    {
        ...
        } elseif ($currency == 'xBLZD' || $currency == 'xBLZD') {
            $balanceB = (int)((((string)$balance) * \App\Http\Controllers\Api\WalletController::rateDollarxBLZD()) * 100);
        ...
        if ($currency == 'btc') {
            ...
            $subtract = bcdiv($content->params->withdraw, \App\Http\Controllers\Api\WalletController::rateDollarxBLZD() * 100, 8);
            $add = bcdiv($content->params->deposit, \App\Http\Controllers\Api\WalletController::rateDollarxBLZD() * 100, 8);
        } elseif ($currency == 'bonus' || $currency == 'BONUS') {
        ...
        if($currency == 'xBLZD'){
            $usd_wager = floatval(number_format($subtract * WalletController::rateDollarxBLZD(), 2, '.', ''));
            $stats->update([
                'bets_xBLZD' => $stats->bets_xBLZD + 1,
                'wins_xBLZD' => $stats->wins_xBLZD + ($profit > 0 ? ($multi < 1 ? 0 : 1) : 0),
                'loss_xBLZD' => $stats->loss_xBLZD + ($profit > 0 ? ($multi < 1 ? 1 : 0) : 1),
                'wagered_xBLZD' => $stats->wagered_xBLZD + $subtract,
                'profit_xBLZD' => $stats->profit_xBLZD + ($profit > 0 ? ($multi < 1 ? -($subtract) : ($profit)) : -($subtract))
            ]);
        }
    ...
    public function getBalance(Request $request)
    {
        ...
        try {
            ...
            if ($currency == 'BTC' || $currency == 'btc') {
                ...
                $balance = (int)((((string)$balance) * \App\Http\Controllers\Api\WalletController::rateDollarxBLZD()) * 100);
            } elseif ($currency == 'bonus' || $currency == 'BONUS') {
                $balance = (int)((((string)$balance) * \App\Http\Controllers\Api\WalletController::rateDollarBonus()) * 100);
            } elseif ($currency == 'usdc' || $currency == 'USDC') {
                $balance = (int)((((string)$balance) * \App\Http\Controllers\Api\WalletController::rateDollarUsdc()) * 100);
